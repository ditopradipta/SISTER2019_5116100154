    ...
    while test:
        try:
            ...
            if text_split[0] == 'list':
                res = server.get_list_dir(text)
            elif text_split[0] == 'create':
                res = server.create_handler(text)
            elif text_split[0] == 'delete':
                res = server.delete_handler(text)
            elif text_split[0] == 'read':
                res = server.read_handler(text)
            elif text_split[0] == 'update':
                res = server.update_handler(text)
            elif text_split[0] == 'cd':
                res = server.connected_device_list()
            elif text_split[0] == 'exit':
                res = server.bye()
                test = False
            elif text_split[0] == 'down':
